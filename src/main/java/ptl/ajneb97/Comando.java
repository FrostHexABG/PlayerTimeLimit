...
public class Comando implements CommandExecutor {
			   ...
			   if(args[0].equalsIgnoreCase("reload")) {
				   ...
				   if (args.length == 2) {
					   if (args[1].equalsIgnoreCase("false")) {
						   plugin.getConfigsManager().getMainConfigManager().setDisabled(false);
						   plugin.getConfigsManager().getMainConfigManager().saveConfig();
						   plugin.getMensajesManager().enviarMensaje(sender, "&aEnabled PlayerTimeLimit.", true);
						   return true;
					   } else if (args[1].equalsIgnoreCase("true")) {
						   plugin.getConfigsManager().getMainConfigManager().setDisabled(true);
						   plugin.getConfigsManager().getMainConfigManager().saveConfig();

						   PlayerManager playerManager = plugin.getPlayerManager();
						   for (Player p : Bukkit.getOnlinePlayers()) {
							   TimeLimitPlayer tlp = playerManager.getPlayerByUUID(p.getUniqueId().toString());
							   if(tlp != null) {
								   tlp.setPlayer(null);
								   tlp.eliminarBossBar();
							   }
						   ...
						   plugin.getMensajesManager().enviarMensaje(sender, "&aDisabled PlayerTimeLimit.", true);
						   return true;
					   } else {
						   plugin.getMensajesManager().enviarMensaje(sender, "&aIncorrect arguments. /playertimelimit disable <true,false>", true);
						   return true;
					   }
				   } else {
					   plugin.getMensajesManager().enviarMensaje(sender, "&aIncorrect arguments. /playertimelimit disable <true,false>", true);
	...
	public void help(CommandSender sender) {
		...
		sender.sendMessage(MensajesManager.getMensajeColor("&8- &c/ptl disable <true,false> &7Disables the plugin."));
		sender.sendMessage(MensajesManager.getMensajeColor(" "));
		sender.sendMessage(MensajesManager.getMensajeColor("&c&m                                                                    "));
	}
